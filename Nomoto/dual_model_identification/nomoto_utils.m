    ...
    methods(Static)
        ...
        function rModel = simulateLinearNomoto(time, delta, K, T, r0)
            ...
            for k = 1:(sampleCount - 1)
                ...
                controlEffect = nomoto_utils.applySignedGain(delta(k), K);
                rModel(k + 1) = decayFactor * rModel(k) + controlEffect * (1 - decayFactor);
            end
        end
        ...
        function rModel = simulateNonlinearNomoto(time, delta, K, T, alpha, r0)
            ...
            for k = 1:(sampleCount - 1)
                ...
                k1 = nomoto_utils.nonLinearRhs(currentR, controlEffect, T, alpha);
                k2 = nomoto_utils.nonLinearRhs(currentR + 0.5 * dt * k1, controlEffect, T, alpha);
                k3 = nomoto_utils.nonLinearRhs(currentR + 0.5 * dt * k2, controlEffect, T, alpha);
                k4 = nomoto_utils.nonLinearRhs(currentR + dt * k3, controlEffect, T, alpha);

                rModel(k + 1) = currentR + (dt / 6) * (k1 + 2 * k2 + 2 * k3 + k4);
            end
        end

        function value = nonLinearRhs(r, controlEffect, T, alpha)
            % nonLinearRhs
            % 非线性 Nomoto 模型右端函数。

            value = (controlEffect - r - alpha * r.^3) / T;
        end

        function weighted = applySignedGain(delta, K)
            % applySignedGain
            % Accept either a scalar K or a struct with branch gains.
            inputSize = size(delta);
            delta = nomoto_utils.ensureColumn(delta);
            [KPos, KNeg] = nomoto_utils.extractBranchGains(K);

            weighted = zeros(size(delta));
            posMask = delta >= 0;
            negMask = ~posMask;

            if any(posMask) && ~isfinite(KPos)
                error('K_pos is required for nonnegative input samples.');
            end
            if any(negMask) && ~isfinite(KNeg)
                error('K_neg is required for negative input samples.');
            end

            weighted(posMask) = KPos .* delta(posMask);
            weighted(negMask) = KNeg .* delta(negMask);
            weighted = reshape(weighted, inputSize);
        end

        function [KPos, KNeg] = extractBranchGains(K)
            % extractBranchGains
            % Normalize scalar K or struct K into K_pos / K_neg.
            if isnumeric(K)
                if ~isscalar(K)
                    error('K must be a scalar or a struct with K_pos / K_neg.');
                end
                KPos = double(K);
                KNeg = double(K);
                return;
            ...
            sharedK = nomoto_utils.findStructField(K, {'K', 'k'}, NaN);
            KPos = nomoto_utils.findStructField(K, {'K_pos', 'KPos', 'k_pos', 'kpos'}, sharedK);
            KNeg = nomoto_utils.findStructField(K, {'K_neg', 'KNeg', 'k_neg', 'kneg'}, sharedK);

        end

        function value = findStructField(s, fieldNames, defaultValue)
            % findStructField
            % Return the first scalar value found in the candidate fields.
            for i = 1:numel(fieldNames)
                fieldName = fieldNames{i};
                if isfield(s, fieldName) && ~isempty(s.(fieldName))
                    value = double(s.(fieldName));
                    if ~isscalar(value)
                        error('Struct field %s must be scalar.', fieldName);
                    end
                    return;
                end
            end
